...
public class ItemSpawner : MonoBehaviourPun, IPunObservable
{
    ...
    private void Spawn()
    {
        ...
        // 생성된 아이템을 5초 뒤에 파괴
        StartCoroutine(CoDestroyAfter(item, 5f));
    }
    ...
    public void SpawnFromEnemy(Vector3 enemyPosition)
    {
        if (!PhotonNetwork.IsMasterClient)
        {
            return;
        }
        enemyPosition += Vector3.up * 0.5f;


        GameObject selectedItem = items[Random.Range(0, items.Length)];
        GameObject item = PhotonNetwork.Instantiate(selectedItem.name, enemyPosition, Quaternion.identity);
